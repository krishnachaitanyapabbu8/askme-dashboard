...
export default function TokenUsage({ data }) {
  ...
  return (
    <div className="page">
      <PageInsight
        title="Token Usage"
        subtitle="Track LLM token consumption by month, module and processing step to understand cost drivers."
        insight={insights?.tokens}
      />

      {/* KPI Row */}
      <div className="kpi-row">
        <KPICard label="Total Tokens" value={m.totalTokens} format="tokens"
          trend={mom.totalTokens} prevValue={mom?.previous?.totalTokens}
          sub="All tokens consumed in this period" />
        <KPICard label="Queries Tracked" value={m.queriesTracked}
          sub="Bot responses with token data" />
        <KPICard label="Sessions Tracked" value={m.sessionsTracked}
          trend={mom.totalSessions} sub="Sessions with token data" />
        <KPICard label="Avg Tokens / Question" value={m.avgTokensPerQuestion}
          trend={mom.avgTokensPerQuestion} prevValue={mom?.previous?.avgTokensPerQuestion}
          sub="Average cost per user question" />
        <KPICard label="Avg SQL Tokens" value={m.avgSqlTokens}
          sub="Average tokens for SQL generation step" />
      </div>

      {/* Row 1: Total Tokens by Month */}
      <div className="chart-row single">
        <ChartCard title="Total Tokens by Month" minHeight={300}>
          {c.tokensByMonth.every(d => d.totalTokens === 0) ? <EmptyState /> : (
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={c.tokensByMonth} margin={{ top: 10, right: 20, left: 30, bottom: 5 }}>
                <defs>
                ...
                <Area type="monotone" dataKey="totalTokens" name="Total Tokens"
                  stroke="#4472C4" strokeWidth={2.5} fill="url(#gradTokens)" dot={{ r: 4 }}>
                  <LabelList dataKey="totalTokens" position="top" formatter={formatTokenAxis}
                    style={{ fontSize: 10, fill: '#4472C4', fontWeight: 600 }} />
                </Area>
              </AreaChart>
            </ResponsiveContainer>
          )}
      ...
      <div className="chart-row single">
        <ChartCard title="Prompt vs Completion Tokens by Month" minHeight={300}>
          {c.tokenSplitByMonth.every(d => d.promptTokens === 0) ? <EmptyState /> : (
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={c.tokenSplitByMonth} margin={{ top: 10, right: 20, left: 30, bottom: 5 }}>
                <defs>
      ...
      <div className="chart-row">
          ...
          {(n) => {
            ...
            return (
              <ResponsiveContainer width="100%" height={Math.max(280, d.length * BAR_H)}>
                <BarChart layout="vertical" data={d} margin={{ top: 5, right: 70, left: 20, bottom: 25 }}>
        ...
        <ChartCard title="Avg Tokens by LLM Step" scrollable minHeight={320}>
          {c.tokensByStep.length === 0 ? <EmptyState /> : (
            <ResponsiveContainer width="100%" height={Math.max(280, c.tokensByStep.length * BAR_H)}>
              <BarChart layout="vertical" data={c.tokensByStep}
                ...
                <Bar dataKey="avgTokens" name="Avg Tokens" fill="#ED7D31" radius={[0, 3, 3, 0]}>
                  <LabelList dataKey="avgTokens" position="right" formatter={v => v.toLocaleString()}
                    style={{ fontSize: 10, fill: '#64748B' }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
